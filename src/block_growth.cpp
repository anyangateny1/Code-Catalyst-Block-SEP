...
std::mutex BlockGrowth::print_mutex;
    ...
    for (int z = parent_block.z; z < parent_block.z_end; ++z) {
        for (int y = parent_block.y; y < parent_block.y_end; ++y) {
            for (int x = parent_block.x; x < parent_block.x_end; ++x) {
                uncompressed_coords.insert(encode_coord(x, y, z));
            }
        }
    }
...
Block BlockGrowth::fit_block(int width, int height, int depth) {
    auto it = uncompressed_coords.begin();
    if (it != uncompressed_coords.end()) {
        auto [x, y, z] = decode_coord(*it);

        int z_off = z - parent_block.z;
        int y_off = y - parent_block.y;
        int x_off = x - parent_block.x;

        Block b(x, y, z, width, height, depth, model.at(z_off, y_off, x_off), x_off, y_off, z_off);
        grow_block(b);
        mark_compressed(z_off, z_off+b.depth, y_off, y_off+b.height, x_off, x_off+b.width);
        return b;
    }
    ...
    return fit_block(width - 1, height - 1, depth - 1);
}
...
void BlockGrowth::mark_compressed(int z0, int z1, int y0, int y1, int x0, int x1) {
    for (int z = z0; z < z1; ++z) {
        for (int y = y0; y < y1; ++y) {
            for (int x = x0; x < x1; ++x) {
                compressed.at(z, y, x) = 1;

                // Remove coordinate from uncompressed set
                int x_abs = parent_block.x + x;
                int y_abs = parent_block.y + y; 
                int z_abs = parent_block.z + z;
                uncompressed_coords.erase(encode_coord(x_abs, y_abs, z_abs));
            }
        }
    }
}
...
void BlockGrowth::grow_block(Block& b) {
    ...
    while (grown) {
        ...
        if (b.x_offset + b.width < parent_x_end) {
            ...
            for (int z = b.z_offset; z < b.z_offset + b.depth && ok; ++z)
                for (int y = b.y_offset; y < b.y_offset + b.height && ok; ++y) {
                    if (model.at(z, y, x_end) != b.tag ||
                        compressed.at(z, y, x_end) != 0)
                        ok = false;
                }
        ...
        if (b.y_offset + b.height < parent_y_end) {
            ...
            for (int z = b.z_offset; z < b.z_offset + b.depth && ok; ++z)
                for (int x = b.x_offset; x < b.x_offset + b.width && ok; ++x) {
                    if (model.at(z, y_end, x) != b.tag ||
                        compressed.at(z, y_end, x) != 0)
                        ok = false;
                }
        ...
        if (b.z_offset + b.depth < parent_z_end) {
            ...
            for (int y = b.y_offset; y < b.y_offset + b.height && ok; ++y)
                for (int x = b.x_offset; x < b.x_offset + b.width && ok; ++x) {
                    if (model.at(z_end, y, x) != b.tag ||
                        compressed.at(z_end, y, x) != 0)
                        ok = false;
                }
